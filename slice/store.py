...
class Store:
    ...
    def get_user_by_id(self, user_id: str) -> dict[str, Any] | None:
        ...
        row = self.db.execute(
            "SELECT u.id, u.role, u.username, u.email, u.name, u.created_at, sc.login_id "
            "FROM users u "
    ...
    def get_user_by_username(self, username: str) -> dict[str, Any] | None:
        """Fetch user by username, email, ID, or student login ID. Never returns password_hash."""
        raw = username.strip()
        norm = raw.lower()
        stu_prefix = f"STU-{raw.upper()}"
        row = self.db.execute(
            "SELECT u.id, u.role, u.username, u.email, u.name, u.created_at, sc.login_id "
            "FROM users u "
            "LEFT JOIN student_credentials sc ON sc.student_id = u.id "
            ...
            "   OR u.id = ? COLLATE NOCASE "
            "   OR sc.login_id = ? COLLATE NOCASE "
            "   OR sc.login_id = ? COLLATE NOCASE",
            (norm, norm, raw, raw, stu_prefix),
        ).fetchone()
        return dict(row) if row else None

    def authenticate_user(self, username_or_email: str, password: str) -> dict[str, Any] | None:
        """Authenticate user against SQLite password_hash using constant-time verification.
        
        Supports lookup by username, email, user ID, and student login ID.
        Transparently upgrades valid legacy hashes to modern PBKDF2.
        """
        ...
        stu_prefix = f"STU-{raw.upper()}"
        row = self.db.execute(
            "SELECT u.id, u.role, u.username, u.email, u.password_hash, u.name, sc.login_id "
            "FROM users u "
            "LEFT JOIN student_credentials sc ON sc.student_id = u.id "
            "WHERE u.username = ? COLLATE NOCASE "
            "   OR u.email = ? COLLATE NOCASE "
        ...
        # Transparently upgrade legacy/non-standard hashes to modern 3-part PBKDF2 hash upon successful verification
        stored_hash = row["password_hash"]
        if not stored_hash.startswith("pbkdf2:sha256:100000$"):
            try:
