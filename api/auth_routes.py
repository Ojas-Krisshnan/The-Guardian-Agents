...
async def login(
    ...
    """Authenticate either a student (STU-XXXXXX login ID) or any user (username/email/id + password)."""
    ...
    # 1. If password is provided, prioritize password authentication for all accounts
    if req.password:
        ...
        actual_role = user_data["role"].lower().strip()
    ...
    # 2. Passwordless student login via STU- ID or raw alphanumeric code
    student_data = store.authenticate_student_id(identifier)
    if not student_data and not identifier.upper().startswith("STU-"):
    ...
    # If identifier looks like a student ID, provide clear student ID guidance
    if identifier.upper().startswith("STU-") or req.login_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid Student ID. Please check the ID provided by your teacher.",
        )

    # Otherwise password is required for password-protected accounts
    raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Invalid username or password",
    )
