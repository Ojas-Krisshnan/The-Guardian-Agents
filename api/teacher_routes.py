...
MAX_DOCUMENT_UPLOAD_BYTES = 10 * 1024 * 1024  # 10 MB limit


@router.post("/teacher/assessments/extract-document", response_model=ExtractDocumentResponse)
async def api_extract_assessment_document(
    file: UploadFile = File(...),
    title: str = Form(""),
    topic: str = Form(""),
    teacher: User = Depends(require_teacher),
) -> ExtractDocumentResponse:
    """Extract and parse questions, options, answer keys, and concepts from an uploaded document (.pdf, .docx, .doc)."""
    filename = file.filename or ""
    ext = filename.split(".")[-1].lower() if "." in filename else ""
    if ext not in ("pdf", "docx", "doc"):
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported file format '.{ext}'. Supported formats: .pdf, .docx, .doc",
        )
    ...
    if len(content) > MAX_DOCUMENT_UPLOAD_BYTES:
        raise HTTPException(
            status_code=413,
            detail=f"File too large ({len(content)} bytes). Maximum allowed size is 10MB.",
        )
    if not content:
    ...
    suffix = f".{ext}"
    temp_dir = tempfile.mkdtemp(prefix="synapse_doc_")
    temp_path = Path(temp_dir) / f"upload{suffix}"

    try:
        temp_path.write_bytes(content)
        extracted_text = extract_text_from_file(temp_path, original_filename=filename)
        result = parse_questions_from_text(
            extracted_text,
            default_title=title.strip() or (filename.rsplit(".", 1)[0] if filename else ""),
            default_topic=topic.strip() or "",
        )
        return ExtractDocumentResponse(
            title=result.title,
            topic=result.topic,
            questions=[ExtractedQuestionItem(**q.model_dump()) for q in result.questions],
            total_extracted=result.total_extracted,
            warnings=result.warnings,
        )
    except DocumentFormatError as dfe:
        raise HTTPException(status_code=400, detail=str(dfe))
