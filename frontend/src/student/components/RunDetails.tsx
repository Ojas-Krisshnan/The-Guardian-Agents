...
export const RunDetails: React.FC<RunDetailsProps> = ({
  ...
  const isSuspended = run.state === 'awaiting_expert';
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onAdvance}
        disabled={advancing || isTerminal || isSuspended}
        style={{
          width: '100%',
          padding: '0.65rem',
          backgroundColor: isTerminal || isSuspended ? '#9ca3af' : '#2563eb',
          color: '#ffffff',
          border: 'none',
          borderRadius: '6px',
          fontWeight: 600,
          cursor: advancing || isTerminal || isSuspended ? 'not-allowed' : 'pointer',
        }}
      >
        {advancing ? 'Advancing Run...' : isTerminal ? 'Run Finished' : isSuspended ? 'Awaiting Expert Response' : 'Advance Run'}
      </button>
    </div>
  );
};
