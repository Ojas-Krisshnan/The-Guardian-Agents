...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
  ...
  const [asmMode, setAsmMode] = useState<'manual' | 'upload'>('manual');
  ...
  const handleClearQuestion = (idx: number) => {
    setBuilderQuestions((prev) =>
      prev.map((q, i) =>
        i === idx
          ? {
  ...
  const handleCorrectAnswerChange = (qIdx: number, correctIdx: number) => {
    setBuilderQuestions((prev) =>
      prev.map((q, i) =>
        i === qIdx ? { ...q, correct_index: correctIdx, needs_review: false, warning: null } : q
      )
    );
  };

  const handleConceptChange = (qIdx: number, concept: string) => {
    setBuilderQuestions((prev) =>
      prev.map((q, i) => (i === qIdx ? { ...q, concept_id: concept } : q))
    );
  };
  ...
  const handleFileUpload = async (file: File) => {
    ...
    if (!['pdf', 'docx', 'doc'].includes(ext)) {
      setUploadError(`Unsupported file format '.${ext}'. Please upload a .pdf, .docx, or .doc file.`);
      return;
    }
    ...
    setUploadError('');
    setUploadSuccessMessage('');
    setUploadProgressText('Reading your assessment document...');

    try {
      const res = await api.extractAssessmentDocument(file, asmTitle, asmTopic);

      if (res.title && !asmTitle.trim()) {
        setAsmTitle(res.title);
      }
      if (res.topic && !asmTopic.trim()) {
        setAsmTopic(res.topic);
      }
      ...
      if (res.questions && res.questions.length > 0) {
        const mapped = res.questions.map((q, idx) => {
          ...
          let correctIdx = opts.findIndex(
            (o) => o && q.correct_answer && o.trim().toLowerCase() === q.correct_answer.trim().toLowerCase()
          );
          if (correctIdx === -1) {
            if (q.correct_answer && /^[A-D]$/i.test(q.correct_answer.trim())) {
              correctIdx = q.correct_answer.trim().toUpperCase().charCodeAt(0) - 65;
            } else {
              correctIdx = 0;
            }
          }
          return {
            id: `q_ext_${Date.now()}_${idx}`,
            question_text: q.question_text || '',
            options: opts,
            correct_index: correctIdx,
            concept_id: q.concept_id || asmTopic.trim() || 'General Concept',
            needs_review: q.needs_review || !q.correct_answer,
            warning: q.warning || (q.needs_review ? 'Please verify correct answer' : null),
          };
        });
        setBuilderQuestions(mapped);
        setUploadSuccessMessage(
          `Questions extracted successfully! (${mapped.length} questions extracted). Review and edit below before publishing.`
        );
      } else {
        setUploadError(
          res.warnings?.[0] || 'Could not detect structured questions in the document. Please check the document format.'
        );
      }
    } catch (err: any) {
      setUploadError(err.message || 'Failed to extract questions from document.');
    } finally {
      setUploading(false);
      setUploadProgressText('');
    }
  ...
  const handleCreateAssessmentWithQuestions = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < builderQuestions.length; i++) {
      ...
      if (!q.question_text.trim()) {
        setValidationError(`Question ${qNum}: Question text is required.`);
        return;
      }
      ...
      for (let j = 0; j < 4; j++) {
        if (!trimmedOpts[j]) {
          ...
          setValidationError(`Question ${qNum}: Option ${letter} cannot be empty. Exactly 4 options are required.`);
          return;
        }
      }
      const uniqueOpts = new Set(trimmedOpts);
      if (uniqueOpts.size !== 4) {
        setValidationError(`Question ${qNum}: All 4 options must be unique.`);
        return;
      }
      if (q.correct_index < 0 || q.correct_index > 3) {
        setValidationError(`Question ${qNum}: Please select a valid correct answer.`);
        return;
      }
    }
    ...
    try {
      const payloadQuestions = builderQuestions.map((q) => {
        const opts = q.options.map((o) => o.trim());
        const concept = q.concept_id.trim() || asmTopic.trim() || 'General Concept';
        return {
      ...
      const asm = await api.createAssessment(selectedClassroom.id, {
        title: title,
        description: asmTopic.trim(),
        concept_ids: [],
      });

      await api.uploadQuestions(asm.id, { questions: payloadQuestions as any });
      ...
      setValidationError(`Error publishing assessment: ${err.message}`);
    } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* MODAL 2: Create Assessment (Manual Question Builder & File Upload) */}
      {showCreateAsmModal && (
        <div
          style={{
            position: 'fixed',
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginTop: '0.2rem', display: 'block' }}>
                  {selectedClassroom ? `Classroom: ${selectedClassroom.name} • ${selectedClassroom.subject}` : 'Build question sets manually or upload questions from file'}
                </span>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleCreateAssessmentWithQuestions} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>
                    Assessment Topic / Concept
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. Recursion Base Case"
                    value={asmTopic}
                    onChange={(e) => setAsmTopic(e.target.value)}
                    style={{ width: '100%', padding: '0.65rem', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 'var(--radius-sm)', color: '#fff' }}
                  />
                </div>
              </div>
              ...
              >
                <button
                  type="button"
                  id="tab-create-manually"
                  onClick={() => setAsmMode('manual')}
                  style={{
                    flex: 1,
                    padding: '0.6rem 1rem',
                    borderRadius: 'var(--radius-sm)',
                    border: 'none',
                    background: asmMode === 'manual' ? 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)' : 'transparent',
                    color: '#fff',
                    fontWeight: asmMode === 'manual' ? 700 : 500,
                    fontSize: '0.9rem',
                    cursor: 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                ...
                <button
                  type="button"
                  id="tab-upload-file"
                  onClick={() => setAsmMode('upload')}
                  style={{
                    flex: 1,
                    padding: '0.6rem 1rem',
                    borderRadius: 'var(--radius-sm)',
                    border: 'none',
                    background: asmMode === 'upload' ? 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)' : 'transparent',
                    color: '#fff',
                    fontWeight: asmMode === 'upload' ? 700 : 500,
                    fontSize: '0.9rem',
                    cursor: 'pointer',
                    display: 'flex',
                    alignItems: 'center',
              ...
              {asmMode === 'upload' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div
                    ...
                    style={{
                      border: `2px dashed ${isDragging ? 'var(--accent-purple-light)' : 'rgba(255, 255, 255, 0.2)'}`,
                      borderRadius: 'var(--radius-md)',
                      padding: '2rem 1.5rem',
                      textAlign: 'center',
                      background: isDragging ? 'rgba(168, 85, 247, 0.08)' : 'rgba(0, 0, 0, 0.25)',
                      transition: 'all 0.2s ease',
                      display: 'flex',
                      flexDirection: 'column',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.84rem', maxWidth: '480px', margin: '0 auto' }}>
                        Drag & drop a file here or browse from your computer. We'll extract the questions for you and let you review them before publishing.
                      </p>
                    </div>
                    <button
                      type="button"
                      ...
                      disabled={uploading}
                      style={{ background: 'rgba(255, 255, 255, 0.08)', marginTop: '0.25rem' }}
                    >
                      Choose File
                    </button>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                  ...
                  {uploading && (
                    ...
                    >
                      ...
                      <span>{uploadProgressText || 'Extracting questions...'}</span>
                    </div>
                  )}
                  ...
                  {uploadError && (
                    ...
                    >
                      ...
                      <div>
                        <strong>Extraction notice:</strong> {uploadError}
                      </div>
                    </div>
                  )}
              ...
              {/* Questions Section (Both Manual Mode and Extracted Preview) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
                  <div>
                    <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#fff', margin: 0 }}>
                      {asmMode === 'upload' && uploadSuccessMessage
                        ? `Extracted Questions (${builderQuestions.length}) — Review & Edit`
                        : `Questions (${builderQuestions.length})`}
                    </h3>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                      Each question requires question text, 4 distinct options, a correct answer, and a concept.
                    </span>
                  </div>
                  <button
                    type="button"
                    ...
                    style={{ background: 'rgba(255, 255, 255, 0.08)', display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.82rem' }}
                  >
                    <Plus size={15} />
                    Add Question
                  </button>
                ...
                {builderQuestions.length === 0 && (
                  ...
                  >
                    ...
                    <h4 style={{ fontSize: '1rem', fontWeight: 700, color: '#fff', marginBottom: '0.3rem' }}>
                      No questions added yet
                    </h4>
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1rem' }}>
                      Click '+ Add Question' to create your first question, or upload a document to extract questions automatically.
                    </p>
                    <button
                      type="button"
                      className="btn btn-primary"
                      onClick={handleAddQuestion}
                      style={{ background: 'linear-gradient(135deg, #7c3aed 0%, #a855f7 100%)', display: 'inline-flex', alignItems: 'center', gap: '0.4rem', margin: '0 auto' }}
                    >
                      <Plus size={15} />
                      Add Question
                    </button>
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                          <button
                            ...
                            style={{
                              ...
                              border: 'none',
                              color: qIdx === 0 ? 'var(--text-muted)' : '#fff',
                              padding: '0.3rem',
                              borderRadius: '4px',
                          ...
                          <button
                            ...
                            disabled={qIdx === builderQuestions.length - 1}
                            style={{
                              background: 'rgba(255, 255, 255, 0.05)',
                              border: 'none',
                              color: qIdx === builderQuestions.length - 1 ? 'var(--text-muted)' : '#fff',
                              padding: '0.3rem',
                              borderRadius: '4px',
                              cursor: qIdx === builderQuestions.length - 1 ? 'not-allowed' : 'pointer',
                            }}
                          >
                            <ChevronDown size={15} />
                          </button>
                          <button
                            ...
                            onClick={() => handleClearQuestion(qIdx)}
                            style={{
                              background: 'rgba(255, 255, 255, 0.05)',
                              border: 'none',
                              color: 'var(--text-secondary)',
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder="e.g. What is the primary role of a base case in recursion?"
                          style={{
                            width: '100%',
                            padding: '0.65rem',
                          ...
                          return (
                            <div key={letter} style={{ position: 'relative' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center' }}>
                                <span
                                  style={{
                                    background: isCorrect ? 'rgba(16, 185, 129, 0.18)' : 'rgba(255, 255, 255, 0.08)',
                                    color: isCorrect ? 'var(--accent-emerald)' : 'var(--text-secondary)',
                                    ...
                                    fontSize: '0.8rem',
                                    fontWeight: 700,
                                    border: `1px solid ${isCorrect ? 'rgba(16, 185, 129, 0.35)' : 'rgba(255, 255, 255, 0.15)'}`,
                                    borderRight: 'none',
                                  }}
                                >
                                  {letter}
                      ...
                      {/* Correct Answer & Concept Fields */}
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '0.85rem', borderTop: '1px solid rgba(255, 255, 255, 0.08)', paddingTop: '0.85rem' }}>
                        <div>
                          ...
                          >
                            <option value={0}>A — {q.options[0] ? q.options[0].substring(0, 40) : 'Option A'}</option>
                            <option value={1}>B — {q.options[1] ? q.options[1].substring(0, 40) : 'Option B'}</option>
                            <option value={2}>C — {q.options[2] ? q.options[2].substring(0, 40) : 'Option C'}</option>
                            <option value={3}>D — {q.options[3] ? q.options[3].substring(0, 40) : 'Option D'}</option>
                          </select>
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.3rem' }}>
                          ...
                          <input
                            type="text"
                            value={q.concept_id}
                            onChange={(e) => handleConceptChange(qIdx, e.target.value)}
                            placeholder={asmTopic.trim() || 'e.g. Recursion Base Case'}
                            style={{
                              width: '100%',
                              padding: '0.55rem 0.75rem',
                              background: 'rgba(0, 0, 0, 0.45)',
                ...
                {/* Additional Add Question button at bottom if questions exist */}
                {builderQuestions.length > 0 && (
                  <button
                    type="button"
                    className="btn btn-secondary"
