...
def test_wrong_username_and_wrong_password_returns_401(auth_store, client):
    """Verify wrong username + wrong password returns HTTP 401 (never 404)."""
    resp = client.post("/auth/login", json={"username": "completely_fake_user", "password": "completely_fake_password"})
    assert resp.status_code == 401
    assert resp.status_code != 404
    assert resp.json()["detail"] == "Invalid username or password"


def test_login_aliases_do_not_produce_404(auth_store, client):
    """Verify /login, /api/login, and /api/auth/login route properly and never 404."""
    store, db_file = auth_store
    store.create_user(role="teacher", username="prof_alias_test", password="ValidPassword123!", name="Prof Alias")

    for path in ["/auth/login", "/login", "/api/login", "/api/auth/login"]:
        # 1. Invalid credentials on path -> 401, not 404
        bad_resp = client.post(path, json={"username": "prof_alias_test", "password": "WrongPassword"})
        assert bad_resp.status_code == 401, f"Path {path} returned {bad_resp.status_code} instead of 401"
        assert bad_resp.status_code != 404, f"Path {path} returned 404!"
        assert bad_resp.json()["detail"] == "Invalid username or password"

        # 2. Valid credentials on path -> 200
        good_resp = client.post(path, json={"username": "prof_alias_test", "password": "ValidPassword123!"})
        assert good_resp.status_code == 200, f"Path {path} returned {good_resp.status_code} instead of 200"
        assert "token" in good_resp.json()


def test_invalid_credentials_do_not_reveal_whether_account_exists(auth_store, client):
    """Verify account enumeration is prevented: existing account + wrong password and nonexistent account return identical response."""
    store, db_file = auth_store
    store.create_user(role="student", username="existing_student_user", password="RealPassword123!", name="Real Student")

    # Case 1: Existing account + wrong password
    resp_wrong_pw = client.post("/auth/login", json={"username": "existing_student_user", "password": "WrongPassword999"})
    # Case 2: Non-existent account + any password
    resp_fake_user = client.post("/auth/login", json={"username": "non_existent_user_xyz", "password": "AnyPassword123"})
    # Case 3: Non-existent email + any password
    resp_fake_email = client.post("/auth/login", json={"username": "fake@nonexistent.domain", "password": "AnyPassword123"})

    assert resp_wrong_pw.status_code == 401
    assert resp_fake_user.status_code == 401
    assert resp_fake_email.status_code == 401

    assert resp_wrong_pw.json() == resp_fake_user.json() == resp_fake_email.json() == {"detail": "Invalid username or password"}
