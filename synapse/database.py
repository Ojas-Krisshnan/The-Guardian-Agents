...
def verify_password(stored_hash: str, password_to_check: str) -> bool:
    """Verify password against stored password hash using constant-time comparison.
    
    Compatible with:
    - Current 3-part format: pbkdf2:sha256:100000$<salt_hex>$<hash_hex>
    - 4-part format: pbkdf2_sha256$<iters>$<salt>$<hash> or <algo>$<iters>$<salt>$<hash>
    - Legacy 2-part format: <salt_hex>$<hash_hex> and inverted <hash_hex>$<salt_hex>
    - Colon-delimited formats: <salt>:<hash>
    - Legacy salted and unsalted SHA256 hex digests
    """
    ...
    try:
        # 1. Check if colon-separated (and no $)
        delimiter = "$" if "$" in stored_hash else (":" if ":" in stored_hash else None)
        if delimiter:
            parts = stored_hash.split(delimiter)
        else:
            parts = [stored_hash]

        # Case A: 4-part hash (e.g. pbkdf2_sha256$260000$salt$hash or algo$iters$salt$hash)
        if len(parts) == 4:
            algo_part, iters_part, salt_str, hash_str = parts
            iters = int(iters_part) if iters_part.isdigit() else 100_000
            expected_hex = hash_str.lower().strip()
            # Try raw bytes salt (from hex) and utf-8 encoded salt
            for salt_val in ([bytes.fromhex(salt_str)] if len(salt_str) % 2 == 0 and all(c in "0123456789abcdefABCDEF" for c in salt_str) else []) + [salt_str.encode("utf-8")]:
                try:
                    actual_hex = hashlib.pbkdf2_hmac("sha256", pw_bytes, salt_val, iters).hex()
                    if secrets.compare_digest(expected_hex, actual_hex):
                        return True
                except Exception:
                    pass

        # Case B: 3-part hash (e.g. pbkdf2:sha256:100000$salt_hex$hash_hex or algo$salt$hash)
        elif len(parts) == 3:
            ...
            try:
                ...
                actual_hex = hashlib.pbkdf2_hmac("sha256", pw_bytes, salt_bytes, iters).hex()
                if secrets.compare_digest(expected_hex, actual_hex):
                    return True
            ...
            try:
                actual_hex2 = hashlib.pbkdf2_hmac("sha256", pw_bytes, salt_hex.encode("utf-8"), iters).hex()
                if secrets.compare_digest(expected_hex, actual_hex2):
                    return True
            except Exception:
                pass

        # Case C: 2-part hash (salt$hash or hash$salt)
        elif len(parts) == 2:
            p0, p1 = parts[0].strip(), parts[1].strip()

            # Determine which part is likely hash vs salt
            candidates: list[tuple[str, str]] = []
            if len(p0) == 64 and len(p1) != 64:
                candidates.append((p1, p0))  # salt=p1, hash=p0
                candidates.append((p0, p1))
            else:
                candidates.append((p0, p1))  # salt=p0, hash=p1
                candidates.append((p1, p0))

            for salt_candidate, hash_candidate in candidates:
                expected_hex = hash_candidate.lower().strip()

                # Try standard PBKDF2 iterations: 100_000, 10_000, 1_000
                salt_variations = []
                if len(salt_candidate) % 2 == 0 and all(c in "0123456789abcdefABCDEF" for c in salt_candidate):
                    try:
                        salt_variations.append(bytes.fromhex(salt_candidate))
                    except Exception:
                        pass
                salt_variations.append(salt_candidate.encode("utf-8"))

                for iters in (100_000, 10_000, 1_000):
                    for s_val in salt_variations:
                        try:
                            act_hex = hashlib.pbkdf2_hmac("sha256", pw_bytes, s_val, iters).hex()
                            if secrets.compare_digest(expected_hex, act_hex):
                                return True
                        except Exception:
                            pass

                # Try simple salted SHA-256 fallback: salt + password, password + salt
                try:
                    s_bytes = salt_candidate.encode("utf-8")
                    if secrets.compare_digest(expected_hex, hashlib.sha256(s_bytes + pw_bytes).hexdigest()):
                        return True
                    if secrets.compare_digest(expected_hex, hashlib.sha256(pw_bytes + s_bytes).hexdigest()):
                        return True
                except Exception:
                    pass

        # Case D: Single 64-char hex string (plain SHA256)
        elif len(parts) == 1:
            raw_hash = parts[0].strip().lower()
            if len(raw_hash) == 64 and all(c in "0123456789abcdef" for c in raw_hash):
                if secrets.compare_digest(raw_hash, hashlib.sha256(pw_bytes).hexdigest()):
                    return True
